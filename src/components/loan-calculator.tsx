...
export default function LoanCalculator() {
  ...
  const [depositDate, setDepositDate] = useState<Date>(() => {
    // Set default to current date (October 22, 2025)
    return new Date()
  })
  const [paidDates, setPaidDates] = useState<Date[]>(() => {
    // Set default to current date (October 22, 2025)
    const currentDate = new Date()
    return [currentDate] // Start with one date
  })
